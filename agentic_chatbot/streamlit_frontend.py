...
if user_input:
    ...
    # container for both animation and response:
    ...
    # showing initial thinking animation:
    thinking_html = """
    <div class='message-container-assistant'>
        <div>✨</div>
        <div class='assistant-message'>
    ...
    for message_chunk, metadata in chatbot.stream(msg, config=CONFIG, stream_mode='messages'):
        ...
        if hasattr(message_chunk, 'tool_calls') and message_chunk.tool_calls and not tool_called:
            tool_called = True
            # maping tool names to emojis and display names:
            for tool_call in message_chunk.tool_calls:
                tool_name = tool_call['name'].lower()
                if 'duckduckgo' in tool_name or 'search' in tool_name:
                    tool_emojis.append('🔍')
                    tool_names.append('Searching web')
        ...
        # streaming, overriding the animation with actual response:
        if hasattr(message_chunk, 'content') and message_chunk.content:
            if hasattr(message_chunk, 'type') and 'AIMessageChunk' in str(type(message_chunk)):
                full_response += message_chunk.content
                
                # Create message with cursor:
                message_container = f"""
                <div class='message-container-assistant'>
                    <div>✨</div>
